...
from data_agent_baseline.tools.profiling import profile_task_context, search_task_documents
...
def _profile_context(task: PublicTask, action_input: dict[str, Any]) -> ToolExecutionResult:
    del action_input
    return ToolExecutionResult(ok=True, content=profile_task_context(task))
...
def _search_docs(task: PublicTask, action_input: dict[str, Any]) -> ToolExecutionResult:
    ...
    return ToolExecutionResult(ok=True, content=search_task_documents(task, query, max_results=max_results))
...
def create_default_tool_registry() -> ToolRegistry:
    specs = {
        ...
        "profile_context": ToolSpec(
            ...
            description=(
                "Build a bounded deterministic profile of all context files: file inventory, "
                "CSV/JSON schemas and samples, SQLite schemas/statistics, and document headings/previews. "
                "Use this early to plan source selection and answer shape."
            ),
            input_schema={},
        ),
        "search_docs": ToolSpec(
            name="search_docs",
            description="Keyword-search markdown/text documents in context and return top matching chunks.",
            input_schema={"query": "terms from the question or needed entity", "max_results": 5},
        ),
