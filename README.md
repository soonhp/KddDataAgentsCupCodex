...
## Competition Docker Submission

This repository includes a rule-compliant Docker entrypoint for the KDD Cup 2026 Data Agents evaluation environment.

### Runtime contract

The container entrypoint is `dabench-submit` and requires no command-line arguments. It scans `/input/task_<id>` and writes:

```text
/output/task_<id>/prediction.csv
/logs/runtime.log
/logs/summary.json
/logs/traces/<task_id>.json
```

Model configuration is read from official evaluation environment variables:
...
```bash
DABENCH_MAX_WORKERS=4
DABENCH_TASK_TIMEOUT_SECONDS=600
DABENCH_AGENT_MAX_STEPS=16
DABENCH_TEMPERATURE=0.0
DABENCH_TASK_LIMIT=0          # 0 means all tasks
DABENCH_SMOKE_MODE=0          # if model env vars are missing, defaults to smoke mode
```
...
This builds image `kdddataagents:v1` for `linux/amd64`.

### Local smoke test

Without official model env vars, set `DABENCH_SMOKE_MODE=1` to verify the `/input -> /output` contract:

```bash
rm -rf /tmp/dabench_output /tmp/dabench_logs
mkdir -p /tmp/dabench_output /tmp/dabench_logs

docker run --rm --platform linux/amd64 \
  -e DABENCH_SMOKE_MODE=1 \
  -e DABENCH_TASK_LIMIT=3 \
  -v /path/to/public/input:/input:ro \
  -v /tmp/dabench_output:/output \
  -v /tmp/dabench_logs:/logs \
  kdddataagents:v1
```

### Save archive for submission

```bash
TEAM_ID=kdddataagents VERSION=v1 ./scripts/save_submission_image.sh
```

The script creates `kdddataagents_v1.tar.gz` using `docker save`, not `docker export`.

### Local score replica

For public demo data with gold answers, score generated predictions with:

```bash
uv run dabench-score   --pred-dir /tmp/dabench_output   --gold-dir /path/to/public/output   --output-json /tmp/dabench_logs/score.json
```

### Hugging Face Inference Providers check

If you do not have a dedicated vLLM endpoint, first verify whether your Hugging Face token can call Qwen through Inference Providers:
...
The token must have permission to make calls to Inference Providers.
